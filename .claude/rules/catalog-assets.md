...
- **`static.ankama.com` bloque les requêtes d'image portant un en-tête `Referer` d'un domaine tiers** (protection anti-hotlink) — un `<img src="https://static.ankama.com/...">` chargé normalement échoue silencieusement (pas d'erreur réseau visible autrement que l'event `error` de l'`<img>`), alors que la même URL fonctionne très bien ouverte directement ou via `curl` (qui n'envoie pas de Referer). Revérifié le 2026-09-20 : 403 avec `Referer: https://wakfu-companion.com/` sur `/web-test/*.png` (avatars) comme sur `/wakfu/portal/game/item/*` ; les images `/wakfu/portal/game/monster/42/*` renvoient 403 même sans Referer. **Ne plus contourner** avec `referrerpolicy="no-referrer"` : c'est un contournement délibéré d'une mesure technique d'Ankama (`docs/analyse-cgu-2026-09-21.md`, § 3.7 et recommandation 5). Les recours objets (ex-`wakfu-item-image-overrides.data.ts`, supprimé depuis : le seul cas réel, « Eclat » lu au singulier dans le log pour « Eclats » en catalogue, est devenu un alias de nom dans `wakfu-item-name-aliases.data.ts`, appliqué à la construction de l'index du `CatalogService`) et l'illustration de combat (`fight-image.util.ts`) sont passés sur wakassets ce jour-là ; seules les galeries d'avatars fan-art (`avatar-fanart-galleries.data.ts`, `profile`/`profile-page`) hotlinkent encore `static.ankama.com` avec l'attribut — décision du mainteneur (2026-09-20) : fan-art communautaire, pas des œuvres Ankama, l'attribut reste sur ces seules balises et ne s'étend à aucune autre.
...
- **Les icônes `/api/v1/icons/*` sont des fichiers statiques en déploiement, pas la Function** (2026-09-23) : une invocation par icône affichée a épuisé le quota quotidien Workers/Pages (projet en « Fail open » → toute l'API servait `index.html`). `tools/bundle-wakassets-icons.mjs` (workflows de déploiement, après `ng build`) copie les dossiers d'`ALLOWED_FOLDERS` + `_routes.json` (exclusion) + `404.html` imbriqué. Nouveau dossier côté client = l'ajouter à `ALLOWED_FOLDERS` ET redéployer ; ne jamais repasser un chemin très sollicité par une Function sans chiffrer les invocations. Limite 20 000 fichiers par déploiement (≈ 15 200).
...
- [static.ankama.com/wakfu/portal/game/item/](https://static.ankama.com/wakfu/portal/game/item/) — CDN officiel Ankama, plus utilisé pour les objets depuis le 2026-09-20 (recours manuels d'images supprimés, voir les gotchas images ci-dessus).
