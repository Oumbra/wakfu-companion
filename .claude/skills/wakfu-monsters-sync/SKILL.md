...
- Comme pour les objets, `referentiel/monsters_wakfu.json` n'est pas consommé directement par l'app : `src/app/core/data/wakfu-monsters.data.ts` en est une génération figée (pas de script de génération committé à ce jour, contrairement à `wakfu-items.data.ts` qui a `tools/generate-wakfu-items-data.mjs`). Régénérer ce fichier est une tâche séparée non couverte par ce skill — à signaler si les nouveaux monstres/champs doivent apparaître dans l'UI immédiatement.
- Les champs `family` et `isDominant` sont nouveaux dans `monsters_wakfu.json` : `wakfu-monsters.data.ts` (généré à la main) ne les porte pas encore.
