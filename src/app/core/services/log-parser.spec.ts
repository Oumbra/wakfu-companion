...
describe('LogParser — filtrage WARN/ERROR', () => {
  ...
  it("n'interrompt pas une ligne multi-lignes en cours si une ligne WARN/ERROR s'intercale", () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 11:20:33,092 [AWT-EventQueue-0] (Sk:64) - [Trade] Starting an exchange between Anonyme-Joueur1 (id=1) and Oumbra (id=2)',
      ' WARN 11:20:33,100 [AWT-EventQueue-0] (x:1) - bruit sans rapport',
      ' INFO 11:21:41,941 [AWT-EventQueue-0] (buN:252) - [Trade] le joueur Anonyme-Joueur1 donne : 0K ; 1xPoudre (refId=27093) ',
...
describe('LogParser — parsing de base (non-régression)', () => {
  it('parse dégâts, sorts et butin', () => {
    ...
    const lines = [
      ...
      ' INFO 11:58:33,180 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Cra3: -8 PV (Terre)',
...
describe('LogParser — multi-combat (fightId)', () => {
  ...
  it('isole deux combats concurrents : la jointure du second ne pollue pas le premier', () => {
    ...
    const lines = [
      ...
      ' INFO 10:00:05,000 [T] (a:1) - [_FL_] fightId=2 Anonyme-Iop1 breed : 8 [2] isControlledByAI=false obstacleId : -1 join the fight at {P}',
  ...
  it(
    ...
    () => {
      ...
      const lines = [
        ...
        ' INFO 10:00:05,000 [T] (a:1) - [_FL_] fightId=2 Anonyme-Iop1 breed : 8 [2] isControlledByAI=false obstacleId : -1 join the fight at {P}',
        ' INFO 10:00:05,001 [T] (a:1) - [_FL_] fightId=2 Kralaman breed : 9 [-2] isControlledByAI=true obstacleId : -1 join the fight at {P}',
        // Même nom de statut ("Vigilance") appliqué par Anonyme-Iop1, dans le combat 2, à Kralaman.
        ' INFO 10:00:06,000 [T] (a:1) - [Information (combat)] Anonyme-Iop1 lance le sort Vigilance',
        ...
        // combat, ce tag aurait résolu Anonyme-Iop1 (combat 2) comme responsable.
      ...
      // plus) => 'Inconnu', jamais 'Anonyme-Iop1' (combat 2).
      expect(damage).toMatchObject({ target: 'Oumbra', attacker: 'Inconnu', fightId: 1 });
    },
  );
  ...
  it('déclare une défaite via "Lancement de l\'occupation" même sans marqueur "vaincu(e)"', () => {
    ...
    const lines = [
      ...
      ' INFO 12:05:32,797 [T] (a:1) - [_FL_] fightId=5 Anonyme-Cra1 breed : 9 [2] isControlledByAI=false obstacleId : -1 join the fight at {P}',
      ' INFO 12:05:38,613 [T] (a:1) - [Information (combat)] Anonyme-Cra1 est KO !',
...
describe('LogParser — dédoublonnage multi-compte', () => {
  it("ignore un dégât identique répété moins d'une seconde plus tard", () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 15:16:23,058 [T] (a:1) - [Information (combat)] Anonyme-Cra4: -489 PV (Air)',
      ' INFO 15:16:23,064 [T] (a:1) - [Information (combat)] Anonyme-Cra4: -489 PV (Air)',
    ];
    const entries = parseAll(parser, lines);
  ...
  it('ne dédoublonne pas au-delà de la fenêtre de tolérance', () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 15:16:23,000 [T] (a:1) - [Information (combat)] Anonyme-Cra4: -489 PV (Air)',
      ' INFO 15:16:25,000 [T] (a:1) - [Information (combat)] Anonyme-Cra4: -489 PV (Air)',
    ];
    const entries = parseAll(parser, lines);
...
describe('LogParser — échanges multi-lignes', () => {
  it('reconstitue un échange dont le résumé final est réparti sur deux lignes physiques', () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 11:20:33,092 [T] (Sk:64) - [Trade] Starting an exchange between Anonyme-Joueur1 (id=1) and Oumbra (id=2)',
      ' INFO 11:21:41,941 [T] (buN:252) - [Trade] le joueur Anonyme-Joueur1 donne : 0K ; 1xFeuilluchon de Fortune (refId=13360) 1xHavre-Gemme Marchande (refId=4262) ',
  ...
  it('capture les kamas échangés de chaque côté', () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 13:46:13,012 [T] (buN:252) - [Trade] le joueur Anonyme-Joueur1 donne : 20K ; ',
    ...
    expect(trade).toMatchObject({
      sides: [
        { playerName: 'Anonyme-Joueur1', kamas: 20, items: [] },
...
describe('LogParser — soin donné (onglet Soin)', () => {
  it("rattache un soin résultant directement d'un sort au lanceur, pas à la cible soignée", () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 12:34:24,027 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Eniripsa1 lance le sort Revitalisation (Critiques)',
      ' INFO 12:34:24,735 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Feca1: +169 PV (Eau)',
    ];
    const entries = parseAll(parser, lines);
    expect(entries[1]).toEqual({
      kind: 'heal',
      time: '12:34:24,735',
      target: 'Anonyme-Feca1',
  ...
  it("rattache un soin \"Délai\" (effet posé par un sort déjà lancé) à l'auteur du sort d'origine, pas au porteur de l'effet", () => {
    ...
    const lines = [
      ' INFO 12:34:24,027 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Eniripsa1 lance le sort Revitalisation (Critiques)',
      ' INFO 12:34:24,735 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Feca1: +169 PV (Eau)',
      ' INFO 12:34:24,740 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Feca1: Délai (+169 Niv.)',
      ' INFO 12:34:48,657 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Feca1: +338 PV (Neutre) (Délai)',
    ];
    const entries = parseAll(parser, lines);
  ...
  it('rattache un soin passif non suivi (ex. "Digestion") à l\'entité qui le porte, jamais au dernier lanceur de sort connu', () => {
    ...
    const lines = [
      ...
      ' INFO 10:58:19,151 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Ouginak2: +234 PV (Neutre) (Digestion)',
    ];
    const entries = parseAll(parser, lines);
    expect(entries[1]).toEqual({
      kind: 'heal',
      time: '10:58:19,151',
      target: 'Anonyme-Ouginak2',
      attacker: 'Anonyme-Ouginak2',
      spell: 'Digestion',
      element: 'Neutre',
      amount: 234,
      fightId: null,
...
describe('LogParser — armure donnée (onglet Armure)', () => {
  it("ignore une perte d'armure (signe négatif) : seule l'armure donnée est suivie", () => {
    const parser = new LogParser();
    const entries = parseAll(parser, [
      ' INFO 15:38:47,729 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Ouginak5: -276 Armure',
    ]);
    expect(entries).toEqual([]);
  });
  ...
  it("rattache un bouclier de feca (nom d'effet ne reprenant que partiellement le nom du sort) au lanceur du sort", () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 12:35:09,848 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Feca1 lance le sort Orbe défensif',
      ' INFO 12:35:10,856 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Ecaflip2: Bouclier Orbe défensif (+1 892 Niv.)',
      ' INFO 12:35:10,867 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Ecaflip2: 956 Armure (Bouclier Orbe défensif)',
    ];
    const entries = parseAll(parser, lines);
    ...
    expect(armor).toEqual({
      kind: 'armor',
      time: '12:35:10,867',
      target: 'Anonyme-Ecaflip2',
      attacker: 'Anonyme-Feca1',
  ...
  it('rattache une armure passive non suivie (ex. "Art Canin") à l\'entité elle-même, même si un autre combattant vient de lancer un sort juste avant', () => {
    ...
    const lines = [
      ...
      ' INFO 10:59:20,106 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Ouginak2: 605 Armure (Art Canin)',
    ];
    const entries = parseAll(parser, lines);
    const armor = entries.find((e) => e.kind === 'armor');
    expect(armor).toEqual({
  ...
  it('rattache une armure sans tag au dernier lanceur de sort (auto-buff)', () => {
    const parser = new LogParser();
    const lines = [
      ' INFO 10:59:03,137 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Ouginak2 lance le sort Molosse',
      ' INFO 10:59:04,447 [AWT-EventQueue-0] (aPV:174) - [Information (combat)] Anonyme-Ouginak2: 312 Armure',
    ];
    const entries = parseAll(parser, lines);
    expect(entries[1]).toEqual({
      kind: 'armor',
...
describe('LogParser — invocations (voir CLAUDE.md)', () => {
  ...
  it(
    "remonte une CHAÎNE d'invocations jusqu'à son sommet (invocation qui en invoque une autre) " +
      ...
      'vrai fichier (Anonyme-Eniripsa2, fightId 1680001273) : "Druidre" (vrai ennemi) invoque "Résidu" ; ' +
