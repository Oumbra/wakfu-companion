...
export interface CatalogMonsterFamilyEntry {
   ...
   * champ `picture` du référentiel. Sert d'icône au groupe "famille de monstre"
...
export interface CatalogDungeonEntry {
   ...
   * ULTIMATE_BREACH) : normalisé ainsi côté serveur (voir
...
export class CatalogService {
   ...
   * homonymes (25 cas constatés dans le référentiel, ex. "Corbac", "Malopo"). */
  ...
  private async refreshIfNeeded(cachedHash: string | null): Promise<boolean> {
    ...
    if (versionResult.data.indexHash === cachedHash) {
      ...
      // les donjons (ex. type de donjon corrigé) ou les familles de monstre
  ...
  private applyDungeons(rawDungeons: CatalogDungeonEntry[]): void {
    ...
    // encore ré-importée après une mise à jour du référentiel) — bug réel constaté le
