 ...
 * quand le référentiel gagne un monstre absent au moment de l'envoi initial.
 *
 * Ne PEUT PAS se contenter d'un `UPDATE ... WHERE name = X` naïf : le référentiel de monstres
 ...
 * le référentiel dans l'ordre du fichier et où le PREMIER monstre à revendiquer un nom
...
async function main(): Promise<void> {
  ...
  console.log(
    `[backfill-monster-ids] ${resolved.length} résolu(s) via le référentiel, ${unresolved.length} inconnu(s) du référentiel (aucune mise à jour possible pour ceux-là).`,
  );
