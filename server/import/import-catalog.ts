...
interface RawItem {
  ...
  hasRecipe?: boolean;
  // Référence l'`id` d'une entrée du référentiel des catégories — un entier, PAS le libellé fr
...
interface RawDungeon {
  ...
  bracket: number;
  type: WakfuDungeonType;
  // Curé à la main dans le référentiel sous 3 formes possibles selon le donjon : `null`
...
/** Normalise `null | number | number[]` (forme de curation manuelle du référentiel,
...
// Sous-catégorie fine (`category` de chaque objet depuis la refonte du référentiel —
// un id du référentiel des catégories, ex. 6 = "Casques", 26 = "Récoltes du Forestier" —
...
//
// Indexée par `subCategoryId` (id du référentiel des catégories, commenté avec le libellé `fr`
 ...
 * référence directement un id du référentiel des catégories, voir ITEM_SUBCATEGORY_CATALOG. Tout
...
/** Avertit (une fois par id, au démarrage de l'import) si le référentiel des catégories contient une
...
async function main(): Promise<void> {
  ...
  const itemRowsWithDuplicates: ItemRow[] = rawItems
    ...
    .map((item) => ({
      ...
      // `item.category` référence déjà directement un id du référentiel des catégories (voir
