...
export const monsterFamilies = pgTable('monster_families', {
  ...
  es: text('es').notNull(),
  pt: text('pt').notNull(),
  // Champ `picture` du référentiel — `null` pour les familles purement
...
export const monsters = pgTable(
  'monsters',
  {
    ...
    // Butin droppable par ce monstre (champ `loot` du référentiel, tableau d'ids
...
export const dungeons = pgTable(
  'dungeons',
  {
    ...
    // Famille(s) de monstre du donjon/de la brèche — même convention
...
export const fightParticipants = pgTable(
  'fight_participants',
  {
     ...
     * nom, et le référentiel contient des homonymes (ex. "Corbac", "Malopo") que
     ...
     * les jeux de test — `Anonyme-Iop1`, `Anonyme-Cra2`...), c'est donc bien un
