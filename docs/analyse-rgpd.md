...
réécriture d'historique. Aucun écart critique ni majeur. **Bilan au soir du 2026-09-21** : les sept points sont traités — 1, 3, 4, 6, 7 corrigés dans le dépôt, 5 supprimé (canal retiré), 2 vérifié sans donnée de tiers. Hors dépôt restait : le registre (portée « tout visiteur » du comptage IP), l'effacement du `wakfu.log` reçu le 16/09 et le sort de la branche `Dev` — **ces deux derniers sont clos le 2026-09-21** (fichier effacé, `Dev` supprimée, `DATABASE_URL_PREVIEW` pointé sur la branche `preview` pseudonymisée).
...
| 13  | ⚪ | **Deux points documentaires** : (a) l'**art. 14** impose en principe d'informer les personnes dont les données n'ont pas été collectées auprès d'elles — ici les joueurs tiers. L'exception d'effort disproportionné (art. 14.5.b) s'applique manifestement (aucun moyen de joindre un joueur à partir de son seul pseudonyme de jeu ; la politique publique tient lieu de mesure compensatoire), mais le raisonnement doit être **écrit**, comme l'a été l'absence d'AIPD. (b) `RATE_LIMIT_SALT` n'est jamais tourné et, à défaut, `clientIpKey` se replie sur `DATABASE_URL` : la pseudonymisation des IP tient tant que ce secret ne fuit pas. | (b) ✅ **Clos le 2026-09-21** : `RATE_LIMIT_SALT` et `RATE_LIMIT_SALT_PREVIEW` sont bien posés tous les deux (secrets du dépôt, vérifiés par le mainteneur) — le repli sur `DATABASE_URL` ne sert donc jamais, et la rotation reste une mesure d'hygiène sans urgence (effet de bord : compteurs remis à zéro sur la fenêtre en cours). (a) ⏳ **Ouvert, hors dépôt** : note art. 14 d'une page à joindre à la note de mise en balance du §1.3 — l'exception d'effort disproportionné n'est acquise que si l'évaluation qui la retient est écrite (art. 5.2, accountability ; lignes directrices EDPB sur la transparence, WP260). |
