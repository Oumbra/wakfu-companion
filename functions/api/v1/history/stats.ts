 ...
 *
 * Cinq `SELECT` indépendants (pas de transaction requise, driver `neon-http` déjà sans
 ...
 * jointure en lecture seule pour XP/butin), toutes filtrées par le même `(userId, plage)`.
 */
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const [fightTotals, xpRows, lootRows, purchaseTotals, tradeTotals] = await Promise.all([
    db
      .select({
