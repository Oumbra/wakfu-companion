...
- **Un conteneur `overflow-x: auto` rogne AUSSI verticalement** : dès qu'un axe vaut `auto`/`hidden`/`scroll`, l'autre ne peut plus valoir `visible` (CSS Overflow §3). Tout tooltip (`::after`, voir plus haut) d'un élément placé dans une bande défilante horizontalement disparaît donc entièrement — bug réel sur `app-tab-bar` (onglets, bouton « tous les onglets », « ? » d'aide, croix de suppression). Le clipping s'arrêtant à la boîte de **padding**, la parade sans JS est de réserver la hauteur du tooltip en `padding-top` puis de l'annuler par une `margin-top` négative de même valeur : la place existe pour le tooltip, sans changer la position des enfants, la hauteur du conteneur ni le défilement (voir `--tab-bar-tooltip-room` dans `tab-bar.component.css`). Attention à ne padder que le côté d'où sort le tooltip — un `padding-bottom` décalerait tout enfant ancré à `bottom: 0` (ici `.tab-bar-underline`).
