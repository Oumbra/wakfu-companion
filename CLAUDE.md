...
- **Le `gfxId` Ankama est la clé stable reliant les JSON officiels aux CDN d'images tiers** (`raw.githubusercontent.com/Nexus-Hub/Wakfu-Companion/.../items/{gfxId}.png`, `cdn.wakfuli.com/items/{gfxId}.webp`) — les deux indexent par ce même id, vérifié sur plusieurs objets. Utile pour toute extension future du référentiel d'objets (voir `wakfu-items.data.ts`).
...
- [github.com/Nexus-Hub/Wakfu-Companion/tree/master/public/](https://github.com/Nexus-Hub/Wakfu-Companion/tree/master/public/) — dépôt source des images d'objets/monstres consommées en direct via `raw.githubusercontent.com/.../items/{gfxId}.png` (voir `shared/item-icon`, `shared/entity-icon`).
